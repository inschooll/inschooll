...
export default function LeftSidebarWrapper({}) {
  ...
  return (
    <>
      {isActive && <Sidebar className={cn("hidden lg:block")} />}
      {isMobileActive && (
        <>
          <div>
            <Sidebar
              className={cn(
                "lg:hidden absolute left-1 top-1/2 z-10 block h-[99%] -translate-y-1/2 rounded-lg border border-cc-border shadow-md ",
              )}
